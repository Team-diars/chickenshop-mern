  ...
  async (req, res) => {
    ...
    if (exists) {
      const product = await Product.findOneAndUpdate(
        { name },
        { category: category.toLowerCase(), price, image, status: 1 }
      );
    ...
    try {
      ...
      if (duplicatedProduct) {
        ...
        const productUpdated = await Product.findOneAndUpdate(
          { name, status: 0 },
          { name: name.trim(), category: category.toLowerCase(), status: 1 }
        );
        ...
        if (isThereAnyProduct && !isSameProduct) {
          return res.status(500).json({
            errors: [
              { msg: `There is an product with the same name registered` },
            ],
          });
        }
      }
